...
class Aes
{
    /**
     * 加密入口函数
     * @param $input    输入信息的字节数组(16字节)
     * @param $w        二维字节数组的key
     * @return array    加密后返回的16字节数组
     */
     ...
     * @param $password     密码
     * @param $nBits        使用的加密位数(可选值 128, 192, or 256)
     * @param int $keep     如果为1每一次加密后的密文字符不变，如果为0则每次加密的密文都会改变，默认值为0
    ...
    public static function encrypt($plaintext, $password, $nBits, $keep = 0)
    {
        ...
        $cipherTxt = array();  // 密文数组

        for ($b = 0; $b < $blockCount; $b++) {
            for ($c = 0; $c < 4; $c++) $counterBlock[15 - $c] = self::urs($b, $c * 8) & 0xff;
            for ($c = 0; $c < 4; $c++) $counterBlock[15 - $c - 4] = self::urs($b / 0x100000000, $c * 8);

            $cipherCnt = Aes::cipher($counterBlock, $keySchedule);

            $blockLength = $b < $blockCount - 1 ? $blockSize : (strlen($plaintext) - 1) % $blockSize + 1;
            $cipherByte = array();

            for ($i = 0; $i < $blockLength; $i++) {
                $cipherByte[$i] = $cipherCnt[$i] ^ ord(substr($plaintext, $b * $blockSize + $i, 1));
    ...
    public static function decrypt($cipherText, $password, $nBits)
    {
        ...
        for ($b = 0; $b < $nBlocks; $b++) {
            ...
            for ($i = 0; $i < strlen($cipherText[$b]); $i++) {
                $plainTxtByte[$i] = $cipherCnt[$i] ^ ord(substr($cipherText[$b], $i, 1));
