...
class Mail {
    ...
    private static function getPasswordHtml($link)
    {
        $html='
            ...
            <p style="font-family: microsoft yahei;">&nbsp;请点击以下链接完成密码找回：</p>
            <a href="'.$link.'" style="font-family: microsoft yahei;">'.$link.'</a></div>
    ...
    private static function getValidateHtml($link)
    {
        $html='
            ...
            <p style="font-family: microsoft yahei;">&nbsp;请点击以下链接完成邮箱认证：</p>
            <a href="'.$link.'" style="font-family: microsoft yahei;">'.$link.'</a></div>
     ...
     * @return array
     */
    public static function  sendPasswordMail($email,$link)
    {
        $url = 'http://sendcloud.sohu.com/webapi/mail.send.json';
        //不同于登录SendCloud站点的帐号，您需要登录后台创建发信子帐号，使用子帐号和密码才可以进行邮件的发送。
        $param = [
            ...
            'to' => $email,
            'subject' => '随游网找回密码',
            'html' => self::getPasswordHtml($link)
        ];
     ...
     * @return array
     */
    public static function  sendValidateMail($email,$link)
    {
        $url = 'http://sendcloud.sohu.com/webapi/mail.send.json';
        //不同于登录SendCloud站点的帐号，您需要登录后台创建发信子帐号，使用子帐号和密码才可以进行邮件的发送。
        $param = [
            ...
            'to' => $email,
            'subject' => '随游网验证邮箱',
            'html' => self::getValidateHtml($link)
        ];
