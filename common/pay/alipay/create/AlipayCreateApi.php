...
class AlipayCreateApi {
    ...
    private $notify_url = "http://www.suiuu.com/pay/alipay-return";

    //页面跳转同步通知页面路径
    private $return_url = "http://www.suiuu.com/pay/alipay-return";
    ...
    public function createOrder(UserOrderInfo $order,UserBase $userBase)
    {
        ...
        $this->subject=str_replace(" ","",$tripInfo['title']);
        $this->price=0.01;//$order->totalPrice;
        $this->body=str_replace(" ","",$tripInfo['intro']);;//暂时写成随游详情 详情内容中不能有空格等参数
        $this->show_url=\Yii::$app->params['base_dir']."/view-trip/info?trip=".$tripInfo['tripId'];
        $this->receive_name=$userBase->nickname;
