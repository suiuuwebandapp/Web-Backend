...
use app\modules\v1\entity\UserOrderComment;
use app\modules\v1\entity\UserOrderInfo;
use app\modules\v1\entity\UserOrderPublisher;
use app\modules\v1\entity\UserOrderPublisherCancel;
use app\modules\v1\entity\UserOrderPublisherIgnore;
...
class UserOrderDb extends ProxyDb
{
    ...
    {
        $sql = sprintf("
            SELECT orderId,orderNumber,userId,tripId,personCount,beginDate,startTime,basePrice,servicePrice,totalPrice,serviceInfo,
              tripJsonInfo,createTime,status,isDel FROM user_order_info
    ...
    {
        $sql = sprintf("
            SELECT uoi.orderId,uoi.orderNumber,uoi.userId,uoi.tripId,uoi.personCount,uoi.beginDate,uoi.startTime,uoi.basePrice,uoi.servicePrice,uoi.totalPrice,uoi.serviceInfo,
              uoi.tripJsonInfo,uoi.createTime,uoi.status,uoi.isDel,ub.nickname,ub.phone,ub.areaCode,ub.email,ub.sex,ub.birthday,ub.headImg,ub.hobby,
    ...
    public function getUnConfirmOrderByPublisher($publisherId)
    {
        ...
        $sql = sprintf("
            SELECT uoi.orderId,uoi.orderNumber,uoi.userId,uoi.tripId,uoi.personCount,uoi.beginDate,uoi.startTime,uoi.basePrice,uoi.servicePrice,uoi.totalPrice,uoi.serviceInfo,
              uoi.tripJsonInfo,uoi.createTime,uoi.status,uoi.isDel,ttp.publisherId,ub.nickname,ub.phone,ub.areaCode,ub.email,ub.sex,ub.birthday,ub.headImg,ub.hobby,
    ...
    public function findPublisherByOrderId($orderId)
    {
        ...
        $sql=sprintf("
            SELECT ub.nickname,ub.phone,ub.email,up.userPublisherId,up.userId,up.countryId,up.cityId,up.lon,up.lat,
            up.idCard,up.idCardImg,up.kind,up.tripCount,up.leadCount,up.registerTime,up.lastUpdateTime,up.score
    ...
    {
        $sql = sprintf("
            SELECT orderCommentId,orderId,userId,tripId,publisherId,content,commentTime,tripScore,publisherScore FROM  user_order_comment
            WHERE orderId=:orderId
        ");
     ...
     * @return Page
     */
    public function getAllOrderList($page,$search,$beginTime,$endTime,$status)
    {
        $sql=sprintf("
            FROM user_order_info uoi
        ...
        $this->setSelectInfo(' uoi.orderId,uoi.orderNumber,uoi.userId,uoi.tripId,uoi.personCount,uoi.beginDate,uoi.startTime,uoi.basePrice,uoi.servicePrice,uoi.totalPrice,uoi.serviceInfo,
              uoi.tripJsonInfo,uoi.createTime,uoi.status,uoi.isDel,ub.nickname AS userNickname,ub.phone AS userPhone,ub.email ');
        $this->setSql($sql);
