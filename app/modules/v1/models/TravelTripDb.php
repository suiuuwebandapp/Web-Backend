...
class TravelTripDb extends ProxyDb
{
     ...
     * @return Page|null
     */
    public function getRelateRecommendTrip($page, $countryId, $cityId)
    {
        $sql = sprintf("
            FROM travel_trip
            WHERE 1=1
    ...
    {
        $sql = sprintf("
            SELECT picId,tripId,title,url FROM travel_trip_picture
            WHERE tripId=:tripId
        ");
        $command = $this->getConnection()->createCommand($sql);
    ...
    {
        $sql = sprintf("
            SELECT specialId,tripId,title,info,picUrl FROM travel_trip_special
            WHERE tripId=:tripId
        ");
        $command = $this->getConnection()->createCommand($sql);
    ...
    public function getTravelTripPublisherList($tripId)
    {
        $sql = sprintf("
            ...
            ub.profession,ub.school,ub.intro,ub.info,ub.travelCount,ub.userSign,t.tripPublisherId,t.tripId,t.publisherId,cit.cname as cityName,cty.cname as countryName
    ...
    {
        $sql = sprintf("
            SELECT scenicId,tripId,name,lon,lat FROM travel_trip_scenic
            WHERE tripId=:tripId
        ");
        $command = $this->getConnection()->createCommand($sql);
    ...
    {
        $sql = sprintf("
            SELECT applyId,tripId,publisherId,sendTime,info,status FROM travel_trip_apply
    ...
    public function getTelTripApplyList($tripId, $status)
    {
        ...
        $this->setSelectInfo("applyId,tripId,publisherId,sendTime,info,status");
        $this->setSql($sql);
        $this->findListBySql();
    }
    ...
    {
        $sql = sprintf("
            SELECT  up.userId,up.countryId,up.cityId,up.lon,up.lat,up.idCard,up.idCardImg,up.kind,up.tripCount,up.leadCount,up.registerTime,up.lastUpdateTime,up.score,tta.applyId,tta.tripId,tta.publisherId,tta.sendTime,tta.info,tta.status,ub.nickname,ub.phone,ub.areaCode,ub.email,ub.sex,ub.birthday,ub.headImg,ub.hobby,ub.userSign,
    ...
    public function getComment($page, $search)
    {
        ...
        $this->setSql($sql);
        $this->setSelectInfo("userSign,content,rTitle,replayCommentId,supportCount,opposeCount,cTime,tripId,isTravel,rUserSign,b.title,c.nickname");
        return $this->find($page);
    }
    ...
    {
        $sql = sprintf("
            SELECT detailId,tripId,name,type FROM travel_trip_detail
            WHERE tripId=:tripId
        ");
    ...
    {
        $sql = sprintf("
            SELECT hlId,tripId,value FROM travel_trip_highlight
            WHERE tripId=:tripId
        ");
    ...
    public function getTripDesSearchList(Page $page,$search)
    {
        $sql = sprintf("
            FROM
            (
                SELECT c.id,c.cname,c.ename,c.areaCode,t.count FROM  (SELECT countryId,COUNT(tripId) as count FROM travel_trip WHERE status=:status GROUP BY countryId HAVING count>0  ) t

                LEFT JOIN country c ON c.id=t.countryId

                UNION

                SELECT c.id,c.cname,c.ename,c.areaCode,t.count FROM  (SELECT cityId,COUNT(tripId) as count FROM travel_trip WHERE status=:status GROUP by cityId HAVING count>0  ) t
