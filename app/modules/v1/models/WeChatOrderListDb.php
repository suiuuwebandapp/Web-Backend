...
use app\modules\v1\entity\WeChatOrderList;
...
class WeChatOrderListDb extends ProxyDb{
    ...
    public function findWeChatOrderInfoById($wOrderId,$userSign){
        $sql = sprintf("
           SELECT wOrderId,wOrderSite,wOrderTimeList,wOrderContent,wUserSign,wStatus,wRelativeSign,wCreateTime,wLastTime,wOrderNumber,wUserNumber,wDetails,wPhone,openId,wMoney,isDel FROM wechat_order_list WHERE wOrderId=:wOrderId AND isDel=FALSE
        ");
        if(!empty($userSign))
        {
    ...
    {
        $sql = sprintf("
           SELECT wOrderId,wOrderSite,wOrderTimeList,wOrderContent,wUserSign,wStatus,wRelativeSign,wCreateTime,wLastTime,wOrderNumber,wUserNumber,wDetails,wPhone,openId,wMoney,isDel  FROM wechat_order_list WHERE wOrderNumber=:wOrderNumber AND isDel=FALSE
        ");
        if(!empty($userSign))
        {
            $sql.=' AND wUserSign=:wUserSign';
...
WHERE a.wOrderNumber=:wOrderNumber  AND isDel=FALSE ORDER BY a.wOrderId DESC
   ...
   /* public function updateWeChatOrderInfo(WeChatOrderList $weChatOrderList){
    ...
    {
        $sql = sprintf("
           SELECT a.wOrderId,a.wOrderSite,a.wOrderTimeList,a.wOrderContent,a.wUserSign,a.wStatus,a.wRelativeSign,a.wCreateTime,a.wLastTime,a.wOrderNumber,a.wUserNumber,a.wDetails,a.wPhone,a.openId,a.wMoney,a.isDel,c.headImg,c.nickname as nickName,b.headImg as rHeadImg,b.nickname as rNickName,b.phone as rPhone
            FROM wechat_order_list a
            LEFT JOIN user_base b ON a.wRelativeSign=b.userSign
    ...
    public function getWeChatOrderPayList($page,$searchText,$type)
    {
        ...
        $this->setSelectInfo('a.payId,a.orderNumber,a.payNumber,a.type,a.money,c.nickname as nickName,b.wStatus');
        $this->setSql($sql);
        return $this->find($page);
    }
