...
class TravelTripCommentDb extends ProxyDb
{
    ...
    {
        $sql = sprintf("
       SELECT commentId,userSign,content,rTitle,replayCommentId,supportCount,opposeCount,cTime,tripId,isTravel,rUserSign FROM travel_trip_comment WHERE commentId=:commentId
        ");
    ...
    public function getTravelOrderByUserSign($userSign,$tripId)
    {
       ...
       SELECT orderId,orderNumber,userId,tripId,personCount,beginDate,startTime,basePrice,servicePrice,totalPrice,serviceInfo,
              tripJsonInfo,createTime,status,isDel FROM user_order_info b WHERE b.userId=:userSign AND b.tripId=:tripId  AND (b.`status`= :successStatus OR b.`status`= :finishStatus)
        ");
    ...
    public function getCommentUserAndTrip($page,$userSign,$tripId)
    {
        ...
        $this->setParam("userSign", $userSign);
        $this->setParam("tripId", $tripId);
        $this->setSelectInfo('commentId,userSign,content,rTitle,replayCommentId,supportCount,opposeCount,cTime,tripId,isTravel,rUserSign');
        $this->setSql($sql);
        return $this->find($page);
    }
    ...
    public function getCommentByUser($page, $userSign)
    {
        ...
        $this->setSelectInfo('a.commentId,a.userSign,a.content,a.rTitle,a.replayCommentId,a.supportCount,a.opposeCount,a.cTime,a.tripId,a.isTravel,a.rUserSign,b.title,c.nickname as rnickname,c.headImg as rheadImg,d.nickname,d.headImg');
        $this->setSql($sql);
        return $this->find($page);
    }
}