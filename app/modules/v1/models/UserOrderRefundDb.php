...
use app\modules\v1\entity\UserOrderRefund;
use app\modules\v1\entity\UserOrderRefundApply;
...
class UserOrderRefundDb extends ProxyDb{
    ...
    public function getOrderRefundApplyList(Page $page,$search,$status)
    {
        ...
        $this->setSelectInfo(' uof.refundApplyId,uof.orderId,uof.userId,uof.tripId,uof.applyContent,uof.applyTime,uof.replyTime,uof.replyContent,uof.status,uoi.orderNumber,uoi.totalPrice,uoi.createTime,uoi.tripJsonInfo,ub.nickname AS userNickname,ub.phone AS userPhone ');
        $this->setSql($sql);
    ...
    {
        $sql = sprintf("
            SELECT refundApplyId,orderId,userId,tripId,applyContent,applyTime,replyTime,replyContent,status FROM  user_order_refund_apply
            WHERE refundApplyId=:refundApplyId
        ");
    ...
    {
        $sql = sprintf("
            SELECT refundId,refundNumber,accountInfo,userId,orderId,tripId,applyId,refundTime,money,content FROM  user_order_refund
