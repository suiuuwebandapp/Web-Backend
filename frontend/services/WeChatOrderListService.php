...
class WeChatOrderListService extends BaseDb{
    ...
    public function getWeChatOrderListByOrderNumber($orderNumber,$userSign=null)
    {
        try {
            $conn = $this->getConnection();
            $this->weChatOrderListDb=new WeChatOrderListDb($conn);
            return $this->weChatOrderListDb->getWeChatOrderListByOrderNumber($orderNumber,$userSign);
        } catch (Exception $e) {
            throw new Exception('查询用户订购信息异常', Code::FAIL, $e);
        } finally {
            $this->closeLink();
    ...
    public function sysOrderInfo($orderNumber)
    {
        try {
            $conn = $this->getConnection();
            $this->weChatOrderListDb=new WeChatOrderListDb($conn);
            return $this->weChatOrderListDb->sysOrderInfo($orderNumber);
        } catch (Exception $e) {
            throw new Exception('查询订购信息详情异常', Code::FAIL, $e);
        } finally {
            $this->closeLink();
    ...
    public function updateOrderInfo(WeChatOrderList $weChatOrderList)
    {
        try {
            $conn = $this->getConnection();
            $this->weChatOrderListDb=new WeChatOrderListDb($conn);
            return $this->weChatOrderListDb->updateWeChatOrderInfo($weChatOrderList);
        } catch (Exception $e) {
            throw new Exception('变更订单异常', Code::FAIL, $e);
        } finally {
            $this->closeLink();
        }
